require('dotenv').config({ path: '.env.local' });
const { Client } = require('pg');

const url = process.env.MARKETING_DATABASE_URL || '';
console.log('Connection string found:', url ? 'YES' : 'NO');

const match = url.match(/postgresql:\/\/([^:]+):([^@]+)@([^:]+):(\d+)\/(.+)/);

if (!match) {
    console.error('❌ No connection string found or invalid format');
    process.exit(1);
}

const [, user, pass, host, port, database] = match;
const decodedPass = decodeURIComponent(pass);

const client = new Client({
    host,
    port,
    database,
    user,
    password: decodedPass
});

async function forceFix() {
    try {
        await client.connect();
        console.log('✓ Connected to database\n');
        
        // Get current data
        const result = await client.query(
            'SELECT id, county_name, total_firms, total_seats, premium_total, standard_total FROM web_availability_county WHERE state_code = $1 AND county_slug = $2',
            ['FL', 'baker']
        );
        
        if (result.rows.length === 0) {
            console.log('❌ Baker County not found in database');
            await client.end();
            return;
        }
        
        const data = result.rows[0];
        console.log('Current Baker County Data:');
        console.log(JSON.stringify(data, null, 2));
        console.log('');
        
        if (data.total_seats === 72) {
            console.log('❌ ERROR: total_seats is still 72! Applying fix...\n');
            
            // Fix county table
            await client.query(
                `UPDATE web_availability_county 
                 SET total_seats = 2, 
                     premium_total = 1, 
                     standard_total = 1,
                     premium_remaining = 1, 
                     standard_remaining = 1,
                     updated_at = NOW()
                 WHERE state_code = 'FL' AND county_slug = 'baker'`
            );
            console.log('✓ Updated county table');
            
            // Fix practice area seats
            const countyId = data.id;
            const paResult = await client.query(
                `UPDATE web_availability_practice_area_seats 
                 SET cap = 1, 
                     premium = 1, 
                     standard = 0,
                     premium_remaining = 1, 
                     standard_remaining = 0,
                     updated_at = NOW()
                 WHERE county_availability_id = $1
                 RETURNING practice_area_slug, cap`,
                [countyId]
            );
            console.log(`✓ Updated ${paResult.rows.length} practice area records`);
            
            // Verify fix
            const verifyResult = await client.query(
                'SELECT county_name, total_firms, total_seats, premium_total, standard_total, premium_remaining, standard_remaining FROM web_availability_county WHERE state_code = $1 AND county_slug = $2',
                ['FL', 'baker']
            );
            
            console.log('\n✓ Fix applied! New data:');
            console.log(JSON.stringify(verifyResult.rows[0], null, 2));
            
            // Verify practice areas
            const paVerify = await client.query(
                `SELECT practice_area_slug, cap, premium, standard 
                 FROM web_availability_practice_area_seats 
                 WHERE county_availability_id = $1
                 ORDER BY display_order
                 LIMIT 3`,
                [countyId]
            );
            console.log('\n✓ Sample practice area data:');
            paVerify.rows.forEach(pa => {
                console.log(`  ${pa.practice_area_slug}: cap=${pa.cap}, premium=${pa.premium}, standard=${pa.standard}`);
            });
        } else {
            console.log(`✓ total_seats is already correct: ${data.total_seats}`);
        }
        
        await client.end();
        console.log('\n✓ Disconnected from database');
    } catch (err) {
        console.error('❌ Error:', err.message);
        console.error(err.stack);
        await client.end();
        process.exit(1);
    }
}

forceFix();

