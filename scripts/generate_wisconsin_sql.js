...
let countySQL = `-- Wisconsin Parish Data
-- Generated: ${new Date().toISOString()}
-- This script populates Wisconsin countyes with validated law firm counts
-- Note: Wisconsin uses "countyes" instead of counties

BEGIN;

-- Delete existing Wisconsin county data
...
DELETE FROM web_availability_county WHERE state_code = 'WI';

-- Insert Wisconsin countyes
`;

let practiceAreaSQL = `-- Wisconsin Practice Area Data
-- Generated: ${new Date().toISOString()}
...
console.log('\n🔍 Validating Wisconsin county data...\n');
const validationReport = validateStateData('WI', countyDataArray);
...
VALUES 
    ('WI', '${countyData.countySlug}', '${countyData.countyName}', ${countyData.totalFirms}, ${countyData.totalSeats}, 
...
WHERE state_code = 'WI' AND county_slug = '${countyData.countySlug}'
...
// Write SQL files
fs.writeFileSync('supabase/POPULATE_WI_COUNTIES.sql', countySQL);
fs.writeFileSync('supabase/POPULATE_WI_PRACTICE_AREAS.sql', practiceAreaSQL);
...
console.log(`   Files:`);
console.log(`   - supabase/POPULATE_WI_COUNTIES.sql`);
console.log(`   - supabase/POPULATE_WI_PRACTICE_AREAS.sql`);
