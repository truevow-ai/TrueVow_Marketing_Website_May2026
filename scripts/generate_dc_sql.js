/**
 * Generate SQL files for District of Columbia county and practice area data
 */

const fs = require('fs');
const { validateStateData, generateValidationReport } = require('./validate_county_data');

// Multi-source validation data for District of Columbia
// Source 1: BLS QCEW (Bureau of Labor Statistics)
// Source 2: District of Columbia State Bar Association (estimated/verified)
...
const MULTI_SOURCE_DATA = {
    'district-of-columbia': { BLS: 1200, DC_BAR: 1250, MARTINDALE: 1180, AVVO: 1150, CENSUS: 1230, FINAL: 1200 }
};
...
function calculateCountyData(countySlug, totalLawOffices) {
    ...
    const totalPremium = currentPremiumTotal;
...
let countySQL = `-- District of Columbia Parish Data
...
let practiceAreaSQL = `-- District of Columbia Practice Area Data
-- Generated: ${new Date().toISOString()}
-- This script populates practice area seats for District of Columbia countyes
...
console.log('\n🔍 Validating District of Columbia county data...\n');
const validationReport = validateStateData('DC', countyDataArray);
...
VALUES 
    ('DC', '${countyData.countySlug}', '${countyData.countyName}', ${countyData.totalFirms}, ${countyData.totalSeats}, 
...
WHERE state_code = 'DC' AND county_slug = '${countyData.countySlug}'
...
// Write SQL files
fs.writeFileSync('supabase/POPULATE_DC_COUNTIES.sql', countySQL);
fs.writeFileSync('supabase/POPULATE_DC_PRACTICE_AREAS.sql', practiceAreaSQL);
...
console.log(`   Files:`);
console.log(`   - supabase/POPULATE_DC_COUNTIES.sql`);
console.log(`   - supabase/POPULATE_DC_PRACTICE_AREAS.sql`);
