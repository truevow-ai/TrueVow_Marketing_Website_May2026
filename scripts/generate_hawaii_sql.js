...
let countySQL = `-- Hawaii Parish Data
-- Generated: ${new Date().toISOString()}
-- This script populates Hawaii countyes with validated law firm counts
-- Note: Hawaii uses "countyes" instead of counties

BEGIN;

-- Delete existing Hawaii county data
DELETE FROM web_availability_practice_area_seats WHERE county_availability_id IN (SELECT id FROM web_availability_county WHERE state_code = 'HI');
DELETE FROM web_availability_county WHERE state_code = 'HI';

-- Insert Hawaii countyes
`;

let practiceAreaSQL = `-- Hawaii Practice Area Data
-- Generated: ${new Date().toISOString()}
-- This script populates practice area seats for Hawaii countyes
...
console.log('\n🔍 Validating Hawaii county data...\n');
const validationReport = validateStateData('HI', countyDataArray);
...
VALUES 
    ('HI', '${countyData.countySlug}', '${countyData.countyName}', ${countyData.totalFirms}, ${countyData.totalSeats}, 
...
WHERE state_code = 'HI' AND county_slug = '${countyData.countySlug}'
...
// Write SQL files
fs.writeFileSync('supabase/POPULATE_HI_COUNTIES.sql', countySQL);
fs.writeFileSync('supabase/POPULATE_HI_PRACTICE_AREAS.sql', practiceAreaSQL);
...
console.log(`   Files:`);
console.log(`   - supabase/POPULATE_HI_COUNTIES.sql`);
console.log(`   - supabase/POPULATE_HI_PRACTICE_AREAS.sql`);
