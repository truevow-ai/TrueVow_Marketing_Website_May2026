...
let countySQL = `-- Iowa Parish Data
-- Generated: ${new Date().toISOString()}
-- This script populates Iowa countyes with validated law firm counts
-- Note: Iowa uses "countyes" instead of counties

BEGIN;

-- Delete existing Iowa county data
...
DELETE FROM web_availability_county WHERE state_code = 'IA';

-- Insert Iowa countyes
`;

let practiceAreaSQL = `-- Iowa Practice Area Data
-- Generated: ${new Date().toISOString()}
...
console.log('\n🔍 Validating Iowa county data...\n');
const validationReport = validateStateData('IA', countyDataArray);
...
VALUES 
    ('IA', '${countyData.countySlug}', '${countyData.countyName}', ${countyData.totalFirms}, ${countyData.totalSeats}, 
...
WHERE state_code = 'IA' AND county_slug = '${countyData.countySlug}'
...
// Write SQL files
fs.writeFileSync('supabase/POPULATE_IA_COUNTIES.sql', countySQL);
fs.writeFileSync('supabase/POPULATE_IA_PRACTICE_AREAS.sql', practiceAreaSQL);
...
console.log(`   Files:`);
console.log(`   - supabase/POPULATE_IA_COUNTIES.sql`);
console.log(`   - supabase/POPULATE_IA_PRACTICE_AREAS.sql`);
