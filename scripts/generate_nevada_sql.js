/**
 * Generate SQL files for Nevada county and practice area data
 */

const fs = require('fs');
const { validateStateData, generateValidationReport } = require('./validate_county_data');

// Multi-source validation data for Nevada
// Source 1: BLS QCEW (Bureau of Labor Statistics)
// Source 2: Nevada State Bar Association (estimated/verified)
...
let countySQL = `-- Nevada Parish Data
...
let practiceAreaSQL = `-- Nevada Practice Area Data
-- Generated: ${new Date().toISOString()}
-- This script populates practice area seats for Nevada countyes
...
console.log('\n🔍 Validating Nevada county data...\n');
const validationReport = validateStateData('NV', countyDataArray);
...
VALUES 
    ('NV', '${countyData.countySlug}', '${countyData.countyName}', ${countyData.totalFirms}, ${countyData.totalSeats}, 
...
WHERE state_code = 'NV' AND county_slug = '${countyData.countySlug}'
...
// Write SQL files
fs.writeFileSync('supabase/POPULATE_NV_COUNTIES.sql', countySQL);
fs.writeFileSync('supabase/POPULATE_NV_PRACTICE_AREAS.sql', practiceAreaSQL);
...
console.log(`   Files:`);
console.log(`   - supabase/POPULATE_NV_COUNTIES.sql`);
console.log(`   - supabase/POPULATE_NV_PRACTICE_AREAS.sql`);
