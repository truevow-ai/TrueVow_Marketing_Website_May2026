...
let countySQL = `-- Alabama Parish Data
-- Generated: ${new Date().toISOString()}
-- This script populates Alabama countyes with validated law firm counts
-- Note: Alabama uses "countyes" instead of counties

BEGIN;

-- Delete existing Alabama county data
DELETE FROM web_availability_practice_area_seats WHERE county_availability_id IN (SELECT id FROM web_availability_county WHERE state_code = 'AL');
DELETE FROM web_availability_county WHERE state_code = 'AL';

-- Insert Alabama countyes
`;

let practiceAreaSQL = `-- Alabama Practice Area Data
-- Generated: ${new Date().toISOString()}
-- This script populates practice area seats for Alabama countyes
...
console.log('\n🔍 Validating Alabama county data...\n');
const validationReport = validateStateData('AL', countyDataArray);
...
VALUES 
    ('AL', '${countyData.countySlug}', '${countyData.countyName}', ${countyData.totalFirms}, ${countyData.totalSeats}, 
...
WHERE state_code = 'AL' AND county_slug = '${countyData.countySlug}'
...
// Write SQL files
fs.writeFileSync('supabase/POPULATE_AL_COUNTIES.sql', countySQL);
fs.writeFileSync('supabase/POPULATE_AL_PRACTICE_AREAS.sql', practiceAreaSQL);
...
console.log(`   Files:`);
console.log(`   - supabase/POPULATE_AL_COUNTIES.sql`);
console.log(`   - supabase/POPULATE_AL_PRACTICE_AREAS.sql`);
