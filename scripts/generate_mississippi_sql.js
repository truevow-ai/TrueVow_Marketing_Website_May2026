...
let countySQL = `-- Mississippi Parish Data
-- Generated: ${new Date().toISOString()}
-- This script populates Mississippi countyes with validated law firm counts
-- Note: Mississippi uses "countyes" instead of counties

BEGIN;

-- Delete existing Mississippi county data
DELETE FROM web_availability_practice_area_seats WHERE county_availability_id IN (SELECT id FROM web_availability_county WHERE state_code = 'MS');
DELETE FROM web_availability_county WHERE state_code = 'MS';

-- Insert Mississippi countyes
`;

let practiceAreaSQL = `-- Mississippi Practice Area Data
-- Generated: ${new Date().toISOString()}
-- This script populates practice area seats for Mississippi countyes
...
console.log('\n🔍 Validating Mississippi county data...\n');
const validationReport = validateStateData('MS', countyDataArray);
...
VALUES 
    ('MS', '${countyData.countySlug}', '${countyData.countyName}', ${countyData.totalFirms}, ${countyData.totalSeats}, 
...
WHERE state_code = 'MS' AND county_slug = '${countyData.countySlug}'
...
// Write SQL files
fs.writeFileSync('supabase/POPULATE_MS_COUNTIES.sql', countySQL);
fs.writeFileSync('supabase/POPULATE_MS_PRACTICE_AREAS.sql', practiceAreaSQL);
...
console.log(`   Files:`);
console.log(`   - supabase/POPULATE_MS_COUNTIES.sql`);
console.log(`   - supabase/POPULATE_MS_PRACTICE_AREAS.sql`);
