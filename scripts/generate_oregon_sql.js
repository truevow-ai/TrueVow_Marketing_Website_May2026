...
let countySQL = `-- Oregon Parish Data
-- Generated: ${new Date().toISOString()}
-- This script populates Oregon countyes with validated law firm counts
-- Note: Oregon uses "countyes" instead of counties

BEGIN;

-- Delete existing Oregon county data
DELETE FROM web_availability_practice_area_seats WHERE county_availability_id IN (SELECT id FROM web_availability_county WHERE state_code = 'OR');
DELETE FROM web_availability_county WHERE state_code = 'OR';

-- Insert Oregon countyes
`;

let practiceAreaSQL = `-- Oregon Practice Area Data
-- Generated: ${new Date().toISOString()}
-- This script populates practice area seats for Oregon countyes
...
console.log('\n🔍 Validating Oregon county data...\n');
const validationReport = validateStateData('OR', countyDataArray);
...
VALUES 
    ('OR', '${countyData.countySlug}', '${countyData.countyName}', ${countyData.totalFirms}, ${countyData.totalSeats}, 
...
WHERE state_code = 'OR' AND county_slug = '${countyData.countySlug}'
...
// Write SQL files
fs.writeFileSync('supabase/POPULATE_OR_COUNTIES.sql', countySQL);
fs.writeFileSync('supabase/POPULATE_OR_PRACTICE_AREAS.sql', practiceAreaSQL);
...
console.log(`   Files:`);
console.log(`   - supabase/POPULATE_OR_COUNTIES.sql`);
console.log(`   - supabase/POPULATE_OR_PRACTICE_AREAS.sql`);
