...
let countySQL = `-- Rhode Island Parish Data
-- Generated: ${new Date().toISOString()}
-- This script populates Rhode Island countyes with validated law firm counts
-- Note: Rhode Island uses "countyes" instead of counties

BEGIN;

-- Delete existing Rhode Island county data
DELETE FROM web_availability_practice_area_seats WHERE county_availability_id IN (SELECT id FROM web_availability_county WHERE state_code = 'RI');
DELETE FROM web_availability_county WHERE state_code = 'RI';

-- Insert Rhode Island countyes
`;

let practiceAreaSQL = `-- Rhode Island Practice Area Data
-- Generated: ${new Date().toISOString()}
-- This script populates practice area seats for Rhode Island countyes
...
console.log('\n🔍 Validating Rhode Island county data...\n');
const validationReport = validateStateData('RI', countyDataArray);
...
VALUES 
    ('RI', '${countyData.countySlug}', '${countyData.countyName}', ${countyData.totalFirms}, ${countyData.totalSeats}, 
...
WHERE state_code = 'RI' AND county_slug = '${countyData.countySlug}'
...
// Write SQL files
fs.writeFileSync('supabase/POPULATE_RI_COUNTIES.sql', countySQL);
fs.writeFileSync('supabase/POPULATE_RI_PRACTICE_AREAS.sql', practiceAreaSQL);
...
console.log(`   Files:`);
console.log(`   - supabase/POPULATE_RI_COUNTIES.sql`);
console.log(`   - supabase/POPULATE_RI_PRACTICE_AREAS.sql`);
