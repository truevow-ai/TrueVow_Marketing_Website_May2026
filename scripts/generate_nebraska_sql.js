...
let countySQL = `-- Nebraska Parish Data
-- Generated: ${new Date().toISOString()}
-- This script populates Nebraska countyes with validated law firm counts
-- Note: Nebraska uses "countyes" instead of counties

BEGIN;

-- Delete existing Nebraska county data
DELETE FROM web_availability_practice_area_seats WHERE county_availability_id IN (SELECT id FROM web_availability_county WHERE state_code = 'NE');
DELETE FROM web_availability_county WHERE state_code = 'NE';

-- Insert Nebraska countyes
`;

let practiceAreaSQL = `-- Nebraska Practice Area Data
-- Generated: ${new Date().toISOString()}
-- This script populates practice area seats for Nebraska countyes
...
console.log('\n🔍 Validating Nebraska county data...\n');
const validationReport = validateStateData('NE', countyDataArray);
...
VALUES 
    ('NE', '${countyData.countySlug}', '${countyData.countyName}', ${countyData.totalFirms}, ${countyData.totalSeats}, 
...
WHERE state_code = 'NE' AND county_slug = '${countyData.countySlug}'
...
// Write SQL files
fs.writeFileSync('supabase/POPULATE_NE_COUNTIES.sql', countySQL);
fs.writeFileSync('supabase/POPULATE_NE_PRACTICE_AREAS.sql', practiceAreaSQL);
...
console.log(`   Files:`);
console.log(`   - supabase/POPULATE_NE_COUNTIES.sql`);
console.log(`   - supabase/POPULATE_NE_PRACTICE_AREAS.sql`);
