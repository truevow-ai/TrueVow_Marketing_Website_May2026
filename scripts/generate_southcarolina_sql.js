/**
 * Generate SQL files for South Carolina county and practice area data
 */

const fs = require('fs');
const { validateStateData, generateValidationReport } = require('./validate_county_data');

// Multi-source validation data for South Carolina
// Source 1: BLS QCEW (Bureau of Labor Statistics)
// Source 2: South Carolina State Bar Association (estimated/verified)
...
let countySQL = `-- South Carolina Parish Data
...
let practiceAreaSQL = `-- South Carolina Practice Area Data
-- Generated: ${new Date().toISOString()}
...
console.log('\n🔍 Validating South Carolina county data...\n');
const validationReport = validateStateData('SC', countyDataArray);
...
VALUES 
    ('SC', '${countyData.countySlug}', '${countyData.countyName}', ${countyData.totalFirms}, ${countyData.totalSeats}, 
...
WHERE state_code = 'SC' AND county_slug = '${countyData.countySlug}'
...
// Write SQL files
fs.writeFileSync('supabase/POPULATE_SC_COUNTIES.sql', countySQL);
fs.writeFileSync('supabase/POPULATE_SC_PRACTICE_AREAS.sql', practiceAreaSQL);
...
console.log(`   Files:`);
console.log(`   - supabase/POPULATE_SC_COUNTIES.sql`);
console.log(`   - supabase/POPULATE_SC_PRACTICE_AREAS.sql`);
