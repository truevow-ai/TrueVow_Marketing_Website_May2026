...
let countySQL = `-- Vermont Parish Data
-- Generated: ${new Date().toISOString()}
-- This script populates Vermont countyes with validated law firm counts
-- Note: Vermont uses "countyes" instead of counties

BEGIN;

-- Delete existing Vermont county data
DELETE FROM web_availability_practice_area_seats WHERE county_availability_id IN (SELECT id FROM web_availability_county WHERE state_code = 'VT');
DELETE FROM web_availability_county WHERE state_code = 'VT';

-- Insert Vermont countyes
`;

let practiceAreaSQL = `-- Vermont Practice Area Data
-- Generated: ${new Date().toISOString()}
-- This script populates practice area seats for Vermont countyes
...
console.log('\n🔍 Validating Vermont county data...\n');
const validationReport = validateStateData('VT', countyDataArray);
...
VALUES 
    ('VT', '${countyData.countySlug}', '${countyData.countyName}', ${countyData.totalFirms}, ${countyData.totalSeats}, 
...
WHERE state_code = 'VT' AND county_slug = '${countyData.countySlug}'
...
// Write SQL files
fs.writeFileSync('supabase/POPULATE_VT_COUNTIES.sql', countySQL);
fs.writeFileSync('supabase/POPULATE_VT_PRACTICE_AREAS.sql', practiceAreaSQL);
...
console.log(`   Files:`);
console.log(`   - supabase/POPULATE_VT_COUNTIES.sql`);
console.log(`   - supabase/POPULATE_VT_PRACTICE_AREAS.sql`);
