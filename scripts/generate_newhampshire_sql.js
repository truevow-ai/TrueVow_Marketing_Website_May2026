/**
 * Generate SQL files for New Hampshire county and practice area data
 */

const fs = require('fs');
const { validateStateData, generateValidationReport } = require('./validate_county_data');

// Multi-source validation data for New Hampshire
// Source 1: BLS QCEW (Bureau of Labor Statistics)
// Source 2: New Hampshire State Bar Association (estimated/verified)
// Source 3: Martindale-Hubbell (estimated)
// Source 4: Avvo (estimated)
// Source 5: Census Business Patterns (estimated)
// Note: New Hampshire uses "countyes" instead of counties
...
let countySQL = `-- New Hampshire Parish Data
-- Generated: ${new Date().toISOString()}
-- This script populates New Hampshire countyes with validated law firm counts
-- Note: New Hampshire uses "countyes" instead of counties

BEGIN;

-- Delete existing New Hampshire county data
DELETE FROM web_availability_practice_area_seats WHERE county_availability_id IN (SELECT id FROM web_availability_county WHERE state_code = 'NH');
DELETE FROM web_availability_county WHERE state_code = 'NH';

-- Insert New Hampshire countyes
`;

let practiceAreaSQL = `-- New Hampshire Practice Area Data
-- Generated: ${new Date().toISOString()}
-- This script populates practice area seats for New Hampshire countyes
...
console.log('\n🔍 Validating New Hampshire county data...\n');
const validationReport = validateStateData('NH', countyDataArray);
...
VALUES 
    ('NH', '${countyData.countySlug}', '${countyData.countyName}', ${countyData.totalFirms}, ${countyData.totalSeats}, 
...
WHERE state_code = 'NH' AND county_slug = '${countyData.countySlug}'
...
// Write SQL files
fs.writeFileSync('supabase/POPULATE_NH_COUNTIES.sql', countySQL);
fs.writeFileSync('supabase/POPULATE_NH_PRACTICE_AREAS.sql', practiceAreaSQL);
...
console.log(`   Files:`);
console.log(`   - supabase/POPULATE_NH_COUNTIES.sql`);
console.log(`   - supabase/POPULATE_NH_PRACTICE_AREAS.sql`);
