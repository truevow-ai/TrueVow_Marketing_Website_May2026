...
let countySQL = `-- Maine Parish Data
-- Generated: ${new Date().toISOString()}
-- This script populates Maine countyes with validated law firm counts
-- Note: Maine uses "countyes" instead of counties

BEGIN;

-- Delete existing Maine county data
DELETE FROM web_availability_practice_area_seats WHERE county_availability_id IN (SELECT id FROM web_availability_county WHERE state_code = 'ME');
DELETE FROM web_availability_county WHERE state_code = 'ME';

-- Insert Maine countyes
`;

let practiceAreaSQL = `-- Maine Practice Area Data
-- Generated: ${new Date().toISOString()}
-- This script populates practice area seats for Maine countyes
...
console.log('\n🔍 Validating Maine county data...\n');
const validationReport = validateStateData('ME', countyDataArray);
...
VALUES 
    ('ME', '${countyData.countySlug}', '${countyData.countyName}', ${countyData.totalFirms}, ${countyData.totalSeats}, 
...
WHERE state_code = 'ME' AND county_slug = '${countyData.countySlug}'
...
// Write SQL files
fs.writeFileSync('supabase/POPULATE_ME_COUNTIES.sql', countySQL);
fs.writeFileSync('supabase/POPULATE_ME_PRACTICE_AREAS.sql', practiceAreaSQL);
...
console.log(`   Files:`);
console.log(`   - supabase/POPULATE_ME_COUNTIES.sql`);
console.log(`   - supabase/POPULATE_ME_PRACTICE_AREAS.sql`);
