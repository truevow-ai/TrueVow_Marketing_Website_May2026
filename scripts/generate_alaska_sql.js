...
let countySQL = `-- Alaska Parish Data
-- Generated: ${new Date().toISOString()}
-- This script populates Alaska countyes with validated law firm counts
-- Note: Alaska uses "countyes" instead of counties

BEGIN;

-- Delete existing Alaska county data
DELETE FROM web_availability_practice_area_seats WHERE county_availability_id IN (SELECT id FROM web_availability_county WHERE state_code = 'AK');
DELETE FROM web_availability_county WHERE state_code = 'AK';

-- Insert Alaska countyes
`;

let practiceAreaSQL = `-- Alaska Practice Area Data
-- Generated: ${new Date().toISOString()}
-- This script populates practice area seats for Alaska countyes
...
console.log('\n🔍 Validating Alaska county data...\n');
const validationReport = validateStateData('AK', countyDataArray);
...
VALUES 
    ('AK', '${countyData.countySlug}', '${countyData.countyName}', ${countyData.totalFirms}, ${countyData.totalSeats}, 
...
WHERE state_code = 'AK' AND county_slug = '${countyData.countySlug}'
...
// Write SQL files
fs.writeFileSync('supabase/POPULATE_AK_COUNTIES.sql', countySQL);
fs.writeFileSync('supabase/POPULATE_AK_PRACTICE_AREAS.sql', practiceAreaSQL);
...
console.log(`   Files:`);
console.log(`   - supabase/POPULATE_AK_COUNTIES.sql`);
console.log(`   - supabase/POPULATE_AK_PRACTICE_AREAS.sql`);
