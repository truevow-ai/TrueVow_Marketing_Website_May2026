...
let countySQL = `-- Kansas Parish Data
-- Generated: ${new Date().toISOString()}
-- This script populates Kansas countyes with validated law firm counts
-- Note: Kansas uses "countyes" instead of counties

BEGIN;

-- Delete existing Kansas county data
...
DELETE FROM web_availability_county WHERE state_code = 'KS';

-- Insert Kansas countyes
`;

let practiceAreaSQL = `-- Kansas Practice Area Data
-- Generated: ${new Date().toISOString()}
...
console.log('\n🔍 Validating Kansas county data...\n');
const validationReport = validateStateData('KS', countyDataArray);
...
VALUES 
    ('KS', '${countyData.countySlug}', '${countyData.countyName}', ${countyData.totalFirms}, ${countyData.totalSeats}, 
...
WHERE state_code = 'KS' AND county_slug = '${countyData.countySlug}'
...
// Write SQL files
fs.writeFileSync('supabase/POPULATE_KS_COUNTIES.sql', countySQL);
fs.writeFileSync('supabase/POPULATE_KS_PRACTICE_AREAS.sql', practiceAreaSQL);
...
console.log(`   Files:`);
console.log(`   - supabase/POPULATE_KS_COUNTIES.sql`);
console.log(`   - supabase/POPULATE_KS_PRACTICE_AREAS.sql`);
