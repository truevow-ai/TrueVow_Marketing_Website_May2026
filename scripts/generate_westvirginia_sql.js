/**
 * Generate SQL files for West Virginia county and practice area data
 */

const fs = require('fs');
const { validateStateData, generateValidationReport } = require('./validate_county_data');

// Multi-source validation data for West Virginia
// Source 1: BLS QCEW (Bureau of Labor Statistics)
// Source 2: West Virginia State Bar Association (estimated/verified)
...
let countySQL = `-- West Virginia Parish Data
...
let practiceAreaSQL = `-- West Virginia Practice Area Data
-- Generated: ${new Date().toISOString()}
-- This script populates practice area seats for West Virginia countyes
...
console.log('\n🔍 Validating West Virginia county data...\n');
const validationReport = validateStateData('WV', countyDataArray);
...
VALUES 
    ('WV', '${countyData.countySlug}', '${countyData.countyName}', ${countyData.totalFirms}, ${countyData.totalSeats}, 
...
WHERE state_code = 'WV' AND county_slug = '${countyData.countySlug}'
...
// Write SQL files
fs.writeFileSync('supabase/POPULATE_WV_COUNTIES.sql', countySQL);
fs.writeFileSync('supabase/POPULATE_WV_PRACTICE_AREAS.sql', practiceAreaSQL);
...
console.log(`   Files:`);
console.log(`   - supabase/POPULATE_WV_COUNTIES.sql`);
console.log(`   - supabase/POPULATE_WV_PRACTICE_AREAS.sql`);
