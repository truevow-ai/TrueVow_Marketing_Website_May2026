/**
 * Generic script to populate practice area seats for ANY state
 * This is reusable - just change the STATE_CODE constant
 */
...
// ============================================
// CONFIGURATION - Change this for each state
// ============================================
const STATE_CODE = process.argv[2] || 'TX';  // Pass state code as argument: node script.js TX

// ============================================
// CONSTANTS (same for all states)
// ============================================
...
// ============================================
// CONNECTION (same as comprehensive_texas_validation_and_update.js)
// ============================================
...
async function populatePracticeAreas() {
    ...
    try {
        ...
        console.log(`📊 Populating practice areas for ${STATE_CODE}...\n`);
        ...
        process.stdout.write(`Processing ${counties.rows.length} counties... `);

        if (counties.rows.length === 0) {
            console.log(`⚠️  No counties found for ${STATE_CODE}. Make sure county data is populated first.`);
            await client.end();
            return;
        }

        // Delete existing practice area seats for this state
        await client.query(`
        ...
        const deleteResult = await client.query(`
            ...
            WHERE county_availability_id IN (SELECT id FROM web_availability_county WHERE state_code = $1)
        `, [STATE_CODE]);
        console.log(`🗑️  Deleted existing practice area seats (${deleteResult.rowCount || 0} rows)\n`);
        ...
        for (const county of counties.rows) {
            ...
            // Insert practice area seats (EXACT same structure as Florida)
            let areasInserted = 0;
            for (const [areaKey, areaData] of Object.entries(practiceAreas)) {
                if (areaData.cap > 0) {
                    await client.query(`
            ...
            console.log(`  ✅ ${county.county_name}: Inserted ${areasInserted} practice areas`);

            processed++;
            if (processed % 5 === 0) {
                console.log(`  Progress: ${processed}/${counties.rows.length} counties processed\n`);
            }
        }
