/**
 * Generate SQL files for Delaware county and practice area data
 */

const fs = require('fs');
const { validateStateData, generateValidationReport } = require('./validate_county_data');

// Multi-source validation data for Delaware
// Source 1: BLS QCEW (Bureau of Labor Statistics)
// Source 2: Delaware State Bar Association (estimated/verified)
...
let countySQL = `-- Delaware Parish Data
...
let practiceAreaSQL = `-- Delaware Practice Area Data
-- Generated: ${new Date().toISOString()}
-- This script populates practice area seats for Delaware countyes
...
console.log('\n🔍 Validating Delaware county data...\n');
const validationReport = validateStateData('DE', countyDataArray);
...
VALUES 
    ('DE', '${countyData.countySlug}', '${countyData.countyName}', ${countyData.totalFirms}, ${countyData.totalSeats}, 
...
WHERE state_code = 'DE' AND county_slug = '${countyData.countySlug}'
...
// Write SQL files
fs.writeFileSync('supabase/POPULATE_DE_COUNTIES.sql', countySQL);
fs.writeFileSync('supabase/POPULATE_DE_PRACTICE_AREAS.sql', practiceAreaSQL);
...
console.log(`   Files:`);
console.log(`   - supabase/POPULATE_DE_COUNTIES.sql`);
console.log(`   - supabase/POPULATE_DE_PRACTICE_AREAS.sql`);
