...
let countySQL = `-- Oklahoma Parish Data
-- Generated: ${new Date().toISOString()}
-- This script populates Oklahoma countyes with validated law firm counts
-- Note: Oklahoma uses "countyes" instead of counties

BEGIN;

-- Delete existing Oklahoma county data
DELETE FROM web_availability_practice_area_seats WHERE county_availability_id IN (SELECT id FROM web_availability_county WHERE state_code = 'OK');
DELETE FROM web_availability_county WHERE state_code = 'OK';

-- Insert Oklahoma countyes
`;

let practiceAreaSQL = `-- Oklahoma Practice Area Data
-- Generated: ${new Date().toISOString()}
-- This script populates practice area seats for Oklahoma countyes
...
console.log('\n🔍 Validating Oklahoma county data...\n');
const validationReport = validateStateData('OK', countyDataArray);
...
VALUES 
    ('OK', '${countyData.countySlug}', '${countyData.countyName}', ${countyData.totalFirms}, ${countyData.totalSeats}, 
...
WHERE state_code = 'OK' AND county_slug = '${countyData.countySlug}'
...
// Write SQL files
fs.writeFileSync('supabase/POPULATE_OK_COUNTIES.sql', countySQL);
fs.writeFileSync('supabase/POPULATE_OK_PRACTICE_AREAS.sql', practiceAreaSQL);
...
console.log(`   Files:`);
console.log(`   - supabase/POPULATE_OK_COUNTIES.sql`);
console.log(`   - supabase/POPULATE_OK_PRACTICE_AREAS.sql`);
