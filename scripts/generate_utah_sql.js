...
let countySQL = `-- Utah Parish Data
-- Generated: ${new Date().toISOString()}
-- This script populates Utah countyes with validated law firm counts
-- Note: Utah uses "countyes" instead of counties

BEGIN;

-- Delete existing Utah county data
DELETE FROM web_availability_practice_area_seats WHERE county_availability_id IN (SELECT id FROM web_availability_county WHERE state_code = 'UT');
DELETE FROM web_availability_county WHERE state_code = 'UT';

-- Insert Utah countyes
`;

let practiceAreaSQL = `-- Utah Practice Area Data
-- Generated: ${new Date().toISOString()}
-- This script populates practice area seats for Utah countyes
...
console.log('\n🔍 Validating Utah county data...\n');
const validationReport = validateStateData('UT', countyDataArray);
...
VALUES 
    ('UT', '${countyData.countySlug}', '${countyData.countyName}', ${countyData.totalFirms}, ${countyData.totalSeats}, 
...
WHERE state_code = 'UT' AND county_slug = '${countyData.countySlug}'
...
// Write SQL files
fs.writeFileSync('supabase/POPULATE_UT_COUNTIES.sql', countySQL);
fs.writeFileSync('supabase/POPULATE_UT_PRACTICE_AREAS.sql', practiceAreaSQL);
...
console.log(`   Files:`);
console.log(`   - supabase/POPULATE_UT_COUNTIES.sql`);
console.log(`   - supabase/POPULATE_UT_PRACTICE_AREAS.sql`);
