...
# -*- coding: utf-8 -*-
"""
Final fix: Replace the complex function with simple one using local data
"""

import re

filepath = "marketing/county-cap.html"
backup_path = "marketing/county-cap.html.final-simple-fix-backup"
...
# Find the section from "// County data must be defined" to end of script tag
# Replace it with the data definition and simplified function
pattern = r'(// County data must be defined BEFORE.*?</script>)'
match = re.search(pattern, content, re.DOTALL)

if match:
    replacement = '''// County data - defined here so it's available immediately (like apply.html)
    ...
    // Update function to use local checkCountyData
    const originalFunction = window.loadCheckCounties;
    ...
    if (typeof loadCheckCounties === 'undefined') {
        loadCheckCounties = window.loadCheckCounties;
    }</script>'''
    
    content = content[:match.start()] + replacement + content[match.end():]
    
    # Also update the function body to use checkCountyData instead of window.checkCountyData
    content = re.sub(
        r'if \(window\.checkCountyData && window\.checkCountyData\[state\]\)',
        'if (checkCountyData[state])',
        content
    )
    content = re.sub(
        r'counties = window\.checkCountyData\[state\]',
        'counties = checkCountyData[state]',
        content
    )
    content = re.sub(
        r'window\.checkCountyData && window\.checkCountyData\[state\]',
        'checkCountyData[state]',
        content
    )
    
    print("  [OK] Replaced county data initialization and updated function")
    ...
    print("  ✅ Added county data as local const in essential script block")
    print("  ✅ Updated function to use local checkCountyData instead of window.checkCountyData")
    print("  ✅ Simplified function logic to match apply.html pattern")
    print(f"{'='*60}")
